...
ReplicaID HotStuffBase::calcTree(std::vector<std::tuple<NetAddr, pubkey_bt, uint256_t>> &&replicas, std::vector<std::tuple<NetAddr, pubkey_bt, uint256_t>> &&replicas2, bool startup) {
    ...
    if (!startup) {
        vheight = b_lock->height + 1;
        piped_queue.clear();

        failures++;

        // 9 times
        if (failures < 9 && fanout < global_replicas.size()) {
        ...
        else if (failures == 9 && !faulty.empty() && fanout < global_replicas.size()) {
    ...
    auto &leader_addr = std::get<0>(original_replicas[1]);
    ...
    for (size_t i = 0; i < original_replicas.size(); i++) {
        ...
        if (current_hash == leader_hash) {
            HOTSTUFF_LOG_PROTO("New actual leader: %d fanout: %d", i, fanout);
            return i;
        }
    }

    HOTSTUFF_LOG_PROTO("New leader: %d", 0);
    return 0;
}
