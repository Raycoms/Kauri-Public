    ...
    void schedule_next() {
        if (!pending_beats.empty())
        {
            if (locked) {
                ...
                if ( false && hsc->b_piped == nullptr && !hsc->piped_submittted && ((current_time.tv_sec - hsc->last_block_time.tv_sec) * 1000000 + current_time.tv_usec - hsc->last_block_time.tv_usec) / 1000 > hsc->get_config().piped_latency) {
