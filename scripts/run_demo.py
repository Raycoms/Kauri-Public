...
if __name__ == "__main__":
    ...
    for ipEl in ipSet:
        ipElSet = ipEl.split(" ")
        if i == 0 :
            command = "sudo tc qdisc add dev enp5s0f0 root netem delay {}ms limit 40000000 rate {}mbit".format(latency, bandwidth)
        else:
            command = "sudo tc qdisc add dev enp5s0f0 root netem delay {}ms limit 40000000".format(latency)
        i+=1
    ...
    ssh.connect("172.16.52.3",port,user,password,timeout = 10)
    ...
    ssh.exec_command("cd test/libhotstuff && ./examples/hotstuff-client --idx {} --iter -500 --max-async 500 > clientlog{} 2>&1 &".format(1, 1))
