...
if __name__ == "__main__":
    ...
    for ipEl in ipSet:
        ipElSet = ipEl.split(" ")
        if i == 0 :
            command = "sudo tc qdisc add dev enp5s0f0 root netem delay {}ms limit 250000 rate {}mbit".format(latency, bandwidth)
        else:
            command = "sudo tc qdisc add dev enp5s0f0 root netem delay {}ms limit 250000".format(latency)
        i+=1
